...
class IoTService {
  ...
  IoTService(String databaseUrl) {
    _initialize(databaseUrl);
  }

  Future<void> _initialize(String databaseUrl) async {
    try {
      // 1. Silent Login (Anonymous)
  ...
  // ... (KEEP ALL YOUR EXISTING STREAMS AND FUNCTIONS BELOW) ...
  // ... (togglePump, setSchedule, updateApp, getHistoryData - NO CHANGES NEEDED) ...

  // Just ensure you copy/paste the rest of the file content here so you don't lose it.
  ...
  Future<List<List<FlSpot>>> getHistoryData() async {
    ...
    if (snapshot.exists && snapshot.value != null) {
      ...
      for (var entry in entries) {
        ...
        tempSpots.add(FlSpot(index.toDouble(), (data['t'] as num).toDouble()));
        soilSpots.add(FlSpot(index.toDouble(), (data['s'] as num).toDouble()));
        index++;
      }
    }
